...
class testWorldParser(unittest.TestCase):
    ...
    def test_empty(self):
        """Empty Simulation is not accepted"""
    ...
    def test_bad_robot(self):
        """Non-comliant robot is rejected"""
        # No type
        self.assertRaises(XMLFormatError, self.parser.parse_string, """
                          <simulation>
                            <robot color="#000000">
            ...
            self.assertEqual(
                ...
                [Struct({'type': 'robot',
                 ...
                 Struct({'type': tag,
                         ...
                         'color': 0xFF0000,
                         'points': [(1, 2), (1, 3), (2, 2),
                                    (2.25, 2.125), (23.125, 22.25)]})])

        test_polygon('obstacle')
        test_polygon('marker')

    def test_bad_polygon(self):
        """Non-comliant polygon is rejected"""
        def test_polygon(tag):
            # No pose
            self.assertRaises(XMLFormatError, self.parser.parse_string, """
                <simulation>
