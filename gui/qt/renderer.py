...
class QtRenderer(Renderer):
    """An implementation of :class:`~renderer.Renderer` for Qt.
    ...
    def _draw_grid(self):
        ...
        self._painter.drawLines(
            [QtCore.QLineF(xmin, i * self._grid_spacing,
                    xmax, i * self._grid_spacing)
                for i in range(*y_ticks)])
        self._painter.drawLines(
            [QtCore.QLineF(i * self._grid_spacing, ymin,
    ...
    def draw_polygon(self,points):
        ...
        self._painter.drawPolygon(QtGui.QPolygonF([QtCore.QPointF(*point[:2]) for point in points]))

    def draw_ellipse(self, cx, cy, ra, rb = None):
        """Draws an ellipse."""
        if rb is None:
            rb = ra
        self._painter.drawEllipse(QtCore.QRectF(cx-ra,cy-ra,2*ra,2*rb))

    def draw_rectangle(self, x, y, w, h):
        """Draws a rectangle."""
        self._painter.drawRect(QtCore.QRectF(x,y,w,h))
    ...
    def draw_line(self, x1, y1, x2, y2):
        ...
        self._painter.drawLine(QtCore.QLineF(x1,y1,x2,y2))
    ...
    def draw_points(self,points):
        ...
        self._painter.drawPoints(QtGui.QPolygonF([QtCore.QPointF(x,y) for x,y in points]))
