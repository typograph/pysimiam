...
def load_by_name(module_string, path=None):
    """Load a class from a module, specified by *module_string*.
       ...
       E.g. ``C = load_by_name('mymodule.MyClass','path.to.module')``
       is equivalent to ``from path.to.module.mymodule import MyClass as C``.
    """
    global __loaded_modules
    ...
    try:
        ...
        try:
            # Load module
            if path is not None:
                module = __import__(path, globals(),
                                    locals(), [filename]).__dict__[filename]
            else:
            ...
            __loaded_modules = __loaded_modules.union(set(sys.modules)
                                                      - old_modules)
