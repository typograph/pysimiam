...
class Simulator(threading.Thread):
                ...
                try:
                    ...
                    if thing.robot.options is not None:
                        robot = robot_class(thing.robot.pose,
                                            options=Struct(thing.robot.options))
                    else:
                        robot = robot_class(thing.robot.pose)
                    if thing.robot.color is not None:
                        robot.color = thing.robot.color
                    elif len(self.__robots) < 8:
                        robot.color = self.__nice_colors[len(self.__robots)]
                    ...
                    if self.__supervisor_param_cache is not None and \
                       len(self.__supervisor_param_cache) > len(self.__supervisors):
                        supervisor.set_parameters(
                            self.__supervisor_param_cache[len(self.__supervisors)])
                    ...
                    # Create trackers
                    self.__trackers.append(simobject.Path(thing.robot.pose, robot.color))
                except:
                    log(self, "[Simulator.construct_world] Robot creation failed!")
                    raise
    ...
    def __reset_world(self):
        ...
        if self.__world is None:
            return
        self.__supervisor_param_cache = [sv.parameters for sv in self.__supervisors]
        self.__construct_world()
                ...
                if self.__state == RUN or \
                   self.__state == RUN_ONCE:
                    ...
                    for i, robot in enumerate(self.__robots):
                        ...
                        self.__trackers[i].add_point(robot.pose)
                    ...
                    for robot, supervisor in zip(self.__robots, self.__supervisors):
                        inputs = supervisor.execute(robot.state, time_constant)
                        robot.set_inputs(inputs)
                        self.fwd_logqueue()
    ...
    def __draw(self):
        ...
        if self.__robots and self.__center_on_robot:
            ...
            if self.__orient_on_robot:
                self.__renderer.set_screen_center_pose(robot.pose)
            else:
                self.__renderer.set_screen_center_pose(
                    pose.Pose(robot.pose, theta= 0.0))
    ...
    def plotables(self):
        ...
        return {
            "Robot's X coordinate": "robot.pose.x",
            "Robot's Y coordinate": "robot.pose.y",
            "Robot's orientation": "robot.pose.theta",
            "Robot's orientation (degrees)": "robot.pose.theta*57.29578",
            "Estimated X coordinate": "supervisor.robot.pose.x",
            "Estimated Y coordinate": "supervisor.robot.pose.y",
            "Estimated orientation": "supervisor.robot.pose.theta",
            "Estimated orientation (degrees)": "supervisor.robot.pose.theta*57.29578",
    ...
    def __check_collisions(self):
        ...
        if len(collisions) > 0:
            ...
            for (robot, obstacle) in collisions:
                log(self, "Collision with {}".format(obstacle),
                         obj=robot,
                         color=robot.color)
            # end of test code
            return True
