...
class Polygon(SimObject):
    """The polygon is a simobject that gets the envelope supplied
       at construction. It draws itself as a filled polygon.
...
class Cloud(SimObject):
    ...
    def add_point(self, pose):
        """Append a point at *pose* to the collection.
           The orientation of the pose is ignored
        """
        self.points.append((pose.x, pose.y))

    def draw(self, r):
        """Draw a polyline with nodes at all added points,
           using the internal color
        """
...
class Path(Cloud):
    """The path is a simobject that draws itself as a polyline.
       ...
       Ramer-Douglas-Peucker algorithm to reduce the amount of segments
       to draw.
    """
    L_crit = 10  # The length of the last path segment that stays 'exact'
    L_sway = 2   # Number of points from the simplified segment
                 # that are going to be simplified again
    ...
    def add_point(self, pose):
        ...
        """
        if self.points and \
           abs(pose.x - self.points[-1][0]) < 0.001 and \
           abs(pose.y - self.points[-1][1]) < 0.001:
            return
        ...
        if len(self.points) - self.simple_length > 2*Path.L_crit:
            ...
            self.points = self.points[:self.simple_length]
                        + Path.RDP(self.points[self.simple_length:-Path.L_crit])
                        + self.points[-Path.L_crit:]
    ...
    def draw(self, r):
        """Draw a polyline with modes at all added points,
           using the internal color
        """
        r.set_pose(self.pose)  # Reset everything
        r.set_pen(self.color)
