...
from quickserver import quickserver, socket
...
class QuickBot(RealBot, quickserver):
    ...
    def __init__(self, pose, color = 0xFFFFFF, options = None):
        ...
        if options is None:
            ...
            quickserver.__init__(self, "localhost", "localhost", 5005)
        else:
            try:
                quickserver.__init__(self, options.baseIP, options.robotIP, options.port)
