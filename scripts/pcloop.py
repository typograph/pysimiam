...
class PCLoop(threading.Thread):
    ...
    def __construct_world(self):
        ...
        for thing in self.__world:
            thing_type = thing[0]
            if thing_type == 'robot' and self.__robot is None:
                ...
                try:
                    ...
                    if robot_options is not None:
                        self.__robot = robot_class(pose.Pose(robot_pose), options = robot_options)
                    else:
                        self.__robot = robot_class(pose.Pose(robot_pose))
                    if robot_color is not None:
                        self.__robot.set_color(robot_color)
                    else:
                    ...
                    if super_options is not None:
                        self.__supervisor = sup_class(self.__robot.get_pose(), info, options = super_options)
                    else:
                        self.__supervisor = sup_class(self.__robot.get_pose(), info)                        
                    ...
                    # Create trackers
                    self.__tracker = simobject.Path(self.__robot.get_pose(),self.__robot.get_color())
                except:
